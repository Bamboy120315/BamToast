package com.bamboy.bamtoast.bamtoast.btoast;
...
public class BToast extends Toast {
    ...
    public static BToast makeText(Context context, CharSequence... text) {
        ...
        return getToast(context, text_0, text_1, 0, ICONTYPE_NONE);
    }
     ...
     * @param isSucceed 显示【对号图标】还是【叉号图标】
     */
    public static BToast makeText(Context context, CharSequence text, CharSequence subText,
                                  boolean isSucceed) {
        return getToast(context, text, subText, 0, isSucceed ? ICONTYPE_SUCCEED : ICONTYPE_ERROR);
    }
     ...
     * @param time    持续的时间
     */
    public static BToast makeText(Context context, CharSequence text, CharSequence subText, int time) {
        return getToast(context, text, subText, time, ICONTYPE_NONE);
    }
    ...
    public static BToast makeText(Context context, CharSequence text, CharSequence subText, int time,
                                  boolean isSucceed) {
        return getToast(context, text, subText, time, isSucceed ? ICONTYPE_SUCCEED : ICONTYPE_ERROR);
    }
     ...
     */
    public static BToast getToast(Context context, CharSequence text, CharSequence subText, int time,
                                  int iconType) {
        initToast(context, text, subText);
    ...
    private static void initToast(Context context, CharSequence... text) {
        try {
            ...
            toast_text.setText(text[0]);
            try {
                toast_sub = layout.findViewById(R.id.toast_sub);
                if (text.length >= 2 && text[1] != null && !"".equals(text[1].toString())) {
                    toast_sub.setText(text[1]);
                    toast_sub.setVisibility(View.VISIBLE);
                } else {
                    toast_sub.setVisibility(View.GONE);
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
